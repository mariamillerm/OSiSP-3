...
void copy_file(const char *curr_name_1, const char *curr_name_2, int *bytes_count) {
    ...
    if (fs == -1) {
        print_error(getpid(), script_name, "Error while opening file ", curr_name_1);
        return;
    }
    if (fd == -1) {
        print_error(getpid(), script_name,   "Error while opening file ", curr_name_2);
        return;
    }
    *bytes_count = 0;
	...
	print_error(getpid(), script_name,   "Memory out Exception", NULL);
	return;
    }
    ssize_t readAmo, writeAmo;
    while (((readAmo = read(fs,buff,BUFSIZE)) != 0) && (readAmo != -1)) {
	if ((writeAmo = write(fd, buff, (size_t)readAmo)) == -1) {
	    print_error(getpid(), script_name,   "Error while writing in file ", curr_name_2);

	    if (close(fs) == -1) {
        	print_error(getpid(), script_name,   "Error while closing file ", curr_name_1);
        	return;
    	    }
	    if (close(fd) == -1) {
        	print_error(getpid(), script_name,   "Error while closing file ", curr_name_2);
        	return;
    	    }
	    free(buff);
	    return;
	...
	curr_name_2[curr_name_len_2] = 0;
		...
		if ( (stat(curr_name_2, &st2) != 0)) {
			...
			exit(EXIT_SUCCESS); 
                    	...
                    	print_error(getpid(), script_name, "Error while forking", NULL);
                    }

                    proc_count++;   
		} else {
		    print_error(getpid(), script_name,  "File already exists", curr_name_2);
		} 
...
int main(int argc, char *argv[]) {
	...
	if (argc < ARGS_COUNT) {
        	print_error(getpid(),script_name, "Not enough arguments.", 0);
        	return 1;
    	}
	...
	char *dir2_name = realpath(argv[2], NULL); //сюда
    	if (dir1_name == NULL) {
        	print_error(getpid(), script_name, "Error opening directory", argv[1]);
        	return 1;
    	} 
	if (dir2_name == NULL) {
		print_error(getpid(), script_name,  "Error opening directory", argv[2]);
		return 1;
	}

    	MAX_PROC_COUNT = atoi(argv[3]);
    	if (MAX_PROC_COUNT == 0) {
        	print_error(getpid(), script_name, "Invalid MAX_PROC_COUNT value", NULL);
        	return EXIT_FAILURE;
    	}	
